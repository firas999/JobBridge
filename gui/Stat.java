...
public class Stat extends Form {
    ...
    public float calcul_nbr_matiere(ArrayList<Evenement> r,String ch){
         ...
         mat =     serviceEvenement.getInstance().getAllEvenement();

        
    int f=0;
    for(int i=0;i<mat.size();i++){
        if (mat.get(i).getEntreprise().equals(ch)){ f++;}
    }
    return f;
}
...
protected CategorySeries buildCategoryDataset(String title, double[] values) {
    CategorySeries series = new CategorySeries(title);
        series.add("Esprit", this.calcul_nbr_matiere(mat, "Esprit") );
        series.add("Telnet", this.calcul_nbr_matiere(mat, "Telnet") );
         series.add("Axia", this.calcul_nbr_matiere(mat, "Axia") );
          series.add("Sopra", this.calcul_nbr_matiere(mat, "Sopra") );
